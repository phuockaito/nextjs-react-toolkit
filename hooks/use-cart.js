...
import { addToCartReducers, updateCartProduct, deleteCartProduct, postCartAPI } from "@/app/slices";
...
export const useCart = (props) => {
    ...
    const { data: dataCity } = useSWR("city", () => fetcherCity("https://provinces.open-api.vn/api/p/"), {
        revalidateOnFocus: false,
        dedupingInterval: 60 * 60 * 1000,
        revalidateOnMount: !!props,
    });
    ...
    const { data: dataDistrict } = useSWR(
        ...
        { revalidateOnFocus: false, dedupingInterval: 60 * 60 * 1000, revalidateOnMount: !!props?.code_district }
    );
    ...
    const { data: dataCommune } = useSWR(
        ...
        { revalidateOnFocus: false, dedupingInterval: 60 * 60 * 1000, revalidateOnMount: !!props?.code_commune }
    );
